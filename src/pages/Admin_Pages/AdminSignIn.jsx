...
function AdminSignIn() {
  ...
  const { AdminFormData, AdminFormChange, AdminCreateRequest, } = AdminStore();
  ...
  const onFormSubmit = async () => {
    if(!ValidationHelper.IsEmail(AdminFormData.email)) {
      ...
      let res = await AdminCreateRequest(AdminFormData);
      if(res) {
        AdminStore.getState().VerifyFormChange("email", AdminFormData.email);
        navigate("/otp");
        ...
        <div className="w-full max-w-md lg:max-w-lg">
          <div className=" rounded-2xl p-6 sm:p-8 ">
            <img
              ...
              className="w-56 h-64 drop-shadow-sm"
            />
          </div>
        </div>
      </div>
